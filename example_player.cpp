...
/*
  ./play_match.pl test holdem.nolimit.2p.reverse_blinds.game 1000 0 Alice ./example_player Bob ./example_player_python.sh
*/
...
Action act(Game *game, MatchState *state, rng_state_t *rng) {
  ...
  if( isValidAction( game, &(state->state), 0, &action ) ) {
    actionProbs[ a_fold ] = probs[ a_fold ];  //0.06 here
    p += probs[ a_fold ];                   //p = 0.06
  }

  /* consider call */
  action.type = a_call;
  action.size = 0;
  actionProbs[ a_call ] = probs[ a_call ];  //0.47 here
  p += probs[ a_call ];                     //p = 0.53

  /* consider raise */
  int32_t min, max;
  if( raiseIsValid( game, &(state->state), &min, &max ) ) {
    actionProbs[ a_raise ] = probs[ a_raise ];  //0.47 here
    p += probs[ a_raise ];                  //p = 1
  }
